...
class SensAIServer:
    ...
    def process_slot(self, slot_id: int, fn) -> bool:
        if not self.transport.is_ready_slot(slot_id):
            return False
        print(f"[Server] Processing slot {slot_id}")
        input_tensor = self.transport.read_tensor_slot(slot_id)
        ...
        self.transport.write_tensor_slot(slot_id, output_tensor)
        return True

    def run_loop(self, fn, interval: float = 0.001):
        while True:
            any_processed = False
            for slot_id in range(self.transport.num_slots):
