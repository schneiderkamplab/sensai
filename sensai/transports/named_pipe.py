...
class NamedPipeTransport(Transport):
    ...
    def __init__(self, path: str, num_clients: int, is_server: bool = False, debug: bool = True):
        super().__init__(path, num_slots=num_clients, is_server=is_server, debug=debug)
        os.makedirs(path, exist_ok=True)
        self.pipes = {}
        self.open_fds = {}
        for slot_id in range(num_clients):
            c2s = os.path.join(path, f"c2s_{slot_id}")  # client → server
            s2c = os.path.join(path, f"s2c_{slot_id}")  # server → client
            for path in (c2s, s2c):
    ...
    def read_tensor(self, slot_id: int) -> np.ndarray:
        path = self._get_pipe_path(slot_id, direction="read")
        self._debug(f"{self.role} will read tensor from slot {slot_id} at {path}")
        with open(path, "rb") as f:
            ...
            self._debug(f"{self.role} read tensor of shape {shape} with dtype {dtype} from slot {slot_id}")
    ...
    def write_tensor(self, slot_id: int, tensor: np.ndarray | list[np.ndarray]) -> None:
        ...
        self._debug(f"{self.role} will write tensor of shape {tensor.shape} to slot {slot_id} at {path}")
        dtype_enum = self._encode_dtype(tensor.dtype)
        ndim = tensor.ndim
        size = tensor.size
        shape = tensor.shape
        self._debug(f"{self.role} writing tensor with dtype {tensor.dtype}, ndim {ndim}, size {size}, shape {shape}")
        with open(path, "wb") as f:
            ...
            self._debug(f"{self.role} wrote tensor of shape {tensor.shape} to slot {slot_id}")
    ...
    def unlink(self):
        for slot_id in range(self.num_clients):
            for direction in ["read", "write"]:
                path = self._get_pipe_path(slot_id, direction)
                try:
                    os.unlink(path)
